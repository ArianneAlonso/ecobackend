...
export default function LoginRegisterScreen() {
  ...
  const [zone, setZone] = useState(''); // barrio / subregion
  ...
  const getLocationAndZone = async () => {
    ...
    try {
      ...
      if (!geo || geo.length === 0) {
        throw new Error('No se pudo obtener la dirección completa.');
      }

      const data = geo[0];

      // Datos más específicos
      const detectedZone = data.district || data.subregion || 'Desconocida'; // barrio
      const detectedStreet = data.street || 'Desconocida';
      const detectedCity = data.city || data.subregion || 'Desconocida';
      ...
      setErrorMessage(err.message || 'Error al obtener la ubicación. Verifica tu conexión.');
      setZone('Desconocida');
      setStreet('Desconocida');
      setCity('Desconocida');
    } finally {
  ...
  const handleRegister = async () => {
    ...
    try {
      await api.register({
        ...
        zone: `${zone}${street ? ', ' + street : ''}, ${city}`, // Guardamos zona completa
      } as any);
        ...
        {activeTab === 'inicio' ? (
          ...
          <ScrollView style={styles.formSection}>
            ...
            <Text style={{ marginTop: 10, color: '#666', fontSize: 14, marginBottom: 10 }}>
              Zona detectada: {loadingLocation ? 'Cargando...' : `${zone}${street ? ', ' + street : ''}, ${city}`}
            </Text>
